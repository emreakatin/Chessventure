...
namespace ThirteenPixels.Soda.Editor
{
    ...
    public class ScopedVariablePropertyDrawer : PropertyDrawer
    {
        ...
        private static void UpdateObject(SerializedProperty property)
        {
            ...
            do
            {
                field = targetObjectType.GetField(property.propertyPath, BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            }
            while (field == null && targetObjectType.BaseType != null && (targetObjectType = targetObjectType.BaseType) != typeof(Object));
